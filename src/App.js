   ...
   <div>
     ...
     <TimeInput hour12Format value={dateEx1} onChange={(dateString)=>setDateEx1(dateString)}/>
      <span>
          ...
          <span>{`     return <TimeInput value={dateEx2} onChange={(dateString)=>setDateEx2(dateString)}/>`}</span>
          <span>{`}`}</span>
         </div>
       </span>
      <h3>24 hour format Time Input : </h3>
     <TimeInput value={dateEx2} onChange={(dateString)=>setDateEx2(dateString)}/>
        <span>
          <div className="code">
          <span>{`import React, { useEffect, useState } from "react";`}</span>  
          <span>{`import TimeInput from "react-time-picker-input";`}</span>  
          <span>{`function TimeInputEx(){`}</span>
          <span>{`     const [dateEx1,setDateEx1]=useState('10:12')`}</span>
          <span>{`     return <TimeInput hour12Format value={dateEx1} onChange={(dateString)=>setDateEx1(dateString)}/>`}</span>
          <span>{`}`}</span>
         </div>
       </span>
