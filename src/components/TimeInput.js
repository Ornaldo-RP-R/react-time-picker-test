...
import { doubleChar,isTouchDevice } from "./actions";
import "./TimeInput.css"

function TimeInput(props) {
  const { hour12Format, value, onChange, allowDelete } = props;
  const [touchableDevice,setTouchableDevice]=useState(isTouchDevice());
  ...
  const updateTouchDevice=()=>setTouchableDevice(isTouchDevice())
  const toggleAmPm = () => setAmPM(amPm === "AM" ? "PM" : "AM");

  useEffect(() => {
    if (hour !== "" && minute !== "" && !touchableDevice) {
  ...
  return (
    <div className="App">
      <div className="react-time-input-picker">
        {touchableDevice ?<div className="input-time-mobile">
